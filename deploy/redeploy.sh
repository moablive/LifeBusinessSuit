...
#
# (shared.env = infra comum na raiz; .env = específico do app e sobrescreve.)
#
...
# --- Configuração ------------------------------------------------------------
# Este script vive em deploy/, mas os projetos ficam na pasta-mãe
# (LifeBusinessSuit/*/docker-compose.yml). ROOT_DIR = pasta-mãe.
...
# As credenciais ficam em deploy/notify.env (fora do git). Variáveis de
# ambiente de mesmo nome têm precedência. Se DEPLOY_CHAT_ID ficar vazio a
# notificação é apenas ignorada — NUNCA quebra o deploy.
NOTIFY_ENV="$SCRIPT_DIR/notify.env"
# shellcheck disable=SC1090
[[ -f "$NOTIFY_ENV" ]] && { set -a; . "$NOTIFY_ENV"; set +a; }
DEPLOY_NOTIFY_ENABLED="${DEPLOY_NOTIFY_ENABLED:-1}"
DEPLOY_BOT_TOKEN="${DEPLOY_BOT_TOKEN:-}"
DEPLOY_CHAT_ID="${DEPLOY_CHAT_ID:-}"
...
mapfile -t ALL_PROJECTS < <(
  find "$ROOT_DIR" -mindepth 2 -maxdepth 2 -name "$COMPOSE_FILE" -printf '%h\n' \
    | sort
)
...
# --- Redeploy ----------------------------------------------------------------
declare -a RESULTS=()
declare -a RESULTS_PLAIN=()
FAILED=0

deploy_one() {
  local dir="$1" name; name="$(basename "$dir")"
  local -a base=(docker compose)
  # Camada 1: infra comum (shared.env na raiz). Camada 2: .env do app, que
  # sobrescreve. A ORDEM importa — o ÚLTIMO --env-file vence. Sem o shared.env
  # o ${DB_DSN}/${DB_NAME} não interpola e o compose falha de forma explícita.
  [[ -f "$ROOT_DIR/shared.env" ]] && base+=(--env-file "$ROOT_DIR/shared.env")
  ...
  if [[ $rc -eq 0 ]]; then
    ...
    RESULTS_PLAIN+=("❌ $name (exit $rc)")
    FAILED=$((FAILED + 1))
  fi
}

# --- Notificação Telegram ----------------------------------------------------
# Envia um resumo do deploy (sucesso/falha, tempo e host) para o DockerBot.
# Depende de DEPLOY_BOT_TOKEN + DEPLOY_CHAT_ID (deploy/notify.env). Sem eles,
# apenas avisa e segue — não afeta o resultado do deploy.
notify_telegram() {
  [[ "$DEPLOY_NOTIFY_ENABLED" == "1" ]] || return 0
  if [[ -z "$DEPLOY_BOT_TOKEN" || -z "$DEPLOY_CHAT_ID" ]]; then
    warn "Telegram: DEPLOY_CHAT_ID/BOT_TOKEN não configurados (deploy/notify.env) — notificação ignorada."
    return 0
  fi
  command -v curl >/dev/null 2>&1 || { warn "Telegram: 'curl' ausente — notificação ignorada."; return 0; }

  local host emoji headline
  host="$(hostname 2>/dev/null || echo '?')"
  if [[ $FAILED -gt 0 ]]; then
    emoji="❌"; headline="Redeploy com falha(s): $FAILED"
  else
    emoji="✅"; headline="Redeploy concluído"
  fi
  ...
  local body list="" r
  for r in "${RESULTS_PLAIN[@]}"; do list+="$(_esc "$r")"$'\n'; done
  body="${emoji} <b>$(_esc "$headline")</b>"$'\n'
  body+="🖥 <code>$(_esc "$host")</code>  ·  ⏱ ${ELAPSED}s"$'\n\n'
  body+="${list}"

  if curl -fsS --max-time 20 \
        "https://api.telegram.org/bot${DEPLOY_BOT_TOKEN}/sendMessage" \
        --data-urlencode "chat_id=${DEPLOY_CHAT_ID}" \
        --data-urlencode "text=${body}" \
        -d "parse_mode=HTML" -d "disable_web_page_preview=true" >/dev/null 2>&1; then
    ok "Notificação enviada ao Telegram (DockerBot)."
  else
    warn "Telegram: falha ao enviar notificação (verifique token/chat_id)."
  fi
}
